...
//! price. Fully offline — uses a synthetic cache fixture via `$TOKSCOPE_PRICING_CACHE`,
//! never the network.
...
fn codex_summary(cache: Option<&str>) -> Value {
    ...
    match cache {
        Some(rel) => cmd.env("TOKSCOPE_PRICING_CACHE", fixtures().join(rel)),
        None => cmd.env(
            "TOKSCOPE_PRICING_CACHE",
