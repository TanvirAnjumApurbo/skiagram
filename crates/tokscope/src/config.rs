//! User configuration: `<config-dir>/tokscope/config.toml` (per-OS via `directories`).
//!
...
//!
//! Resolution order for the file path: `$TOKSCOPE_CONFIG` (explicit override, also
//! the test hook), else `<config_dir>/tokscope/config.toml`.
//!
...
impl Config {
    /// `<config_dir>/tokscope/config.toml` for this OS, or `None` when the config
    ...
    /// Load configuration, honoring `$TOKSCOPE_CONFIG` then the standard path, and
    /// falling back to defaults on any absence/error (see module docs).
    pub fn load() -> Config {
        if let Ok(p) = std::env::var("TOKSCOPE_CONFIG") {
            if !p.trim().is_empty() {
...
mod tests {
    ...
    fn write_temp(name: &str, body: &str) -> PathBuf {
        ...
        p.push(format!("tokscope-cfg-{}-{name}", std::process::id()));
    ...
    fn missing_file_is_defaults_not_error() {
        let p = std::env::temp_dir().join("tokscope-cfg-does-not-exist.toml");
        let _ = std::fs::remove_file(&p);
