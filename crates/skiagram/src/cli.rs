...
use skiagram_core::analysis::drilldown::build_details;
use skiagram_core::model::Session;
use skiagram_core::pricing::PricingTable;
use skiagram_core::{adapters, analysis::aggregate::Summary};
...
/// [`skiagram_core::analysis::flame::FlameMetric`] (the core crate has no clap
...
impl From<MetricArg> for skiagram_core::analysis::flame::FlameMetric {
...
pub fn run() -> anyhow::Result<()> {
    ...
    match cli.command.unwrap_or(Command::Summary { json: false }) {
        ...
        Command::Context { json } => {
            ...
            let report =
                skiagram_core::analysis::context::profile(&sessions, adapter.id(), cli.since);
            if json {
        ...
        Command::Anomalies { json } => {
            ...
            let report = skiagram_core::analysis::anomaly::detect(
                &sessions,
                &filter,
                adapter.id(),
                &pricing,
        ...
        Command::Classify { json } => {
            let (sessions, _failed) = collect_sessions(adapter.as_ref())?;
            let filter = Filter { since: cli.since };
            let report = skiagram_core::analysis::classify::classify(
                &sessions,
                &filter,
                adapter.id(),
                &pricing,
